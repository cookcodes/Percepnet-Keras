...
def printLayer(f, ft, layer):
    ...
    if len(weights) > 2:
        ...
        if "conv" in layer.name :
            f.write('static const ConvLayer {} = {{\n   {}_weights,\n   {}, {}, {}\n}};\n\n'
                .format(name, name, weights[0].shape[0], weights[0].shape[1],weights[0].shape[2]))
        else:
...
model = load_model(sys.argv[1], custom_objects={'msse': mean_squared_sqrt_error, 'mean_squared_sqrt_error': mean_squared_sqrt_error, 'my_crossentropy': mean_squared_sqrt_error, 'mycost': mean_squared_sqrt_error, 'gbcost': mean_squared_sqrt_error,'rbcost': mean_squared_sqrt_error, 'WeightClip': foo})
